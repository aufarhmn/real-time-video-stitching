...
# Adding Epipolar Geometry
# Adding Delayed Frame Extraction
def extract_frame_after_delay(video_path, delay_seconds=0.5):
...
def calculate_homography_from_delayed_frames(video1_path, video2_path, delay_seconds=1):
...
def stitch_video_frames(video1_path, video2_path, output_path, H, frame_shape):
    ...
    out = cv2.VideoWriter(output_path, fourcc, 20.0, (frame_shape[1] * 2, frame_shape[0]))
    ...
    while True:
        ...
        out.write(final_stitched)

        cv2.imshow("Stitched Video", final_stitched)
